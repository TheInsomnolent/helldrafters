...
const ANALYTICS_DEBUG_MODE = process.env.REACT_APP_ANALYTICS_DEBUG === 'true' || 
...
export const initializeAnalytics = async () => {
  ...
  try {
    ...
    if (ANALYTICS_DEBUG_MODE) {
      ...
      console.log('   3. If no device appears, install "Google Analytics Debugger" Chrome extension');
      console.log('');
      console.log('   ⚠️  Check Network tab for requests to google-analytics.com or analytics.google.com');
    ...
    console.log(`   Measurement ID: ${firebaseConfig.measurementId || 'NOT SET - analytics will not work!'}`);
...
export const isAnalyticsConfigured = () => {
  return !!(firebaseConfig.apiKey && ANALYTICS_ENABLED);
};

export const isAnalyticsDebugMode = () => {
  return ANALYTICS_DEBUG_MODE;
};

export const isAnalyticsConsoleLogging = () => {
  return ANALYTICS_CONSOLE_LOGGING;
};
