...
export const recordSampleChange = (samples, reason, eventName = null) => {
  ...
  const lastSnapshot = analyticsStore.sampleSnapshots[analyticsStore.sampleSnapshots.length - 1] || 
...
export const recordRequisitionChange = (change, playerName, reason) => {
  ...
  const lastSnapshot = analyticsStore.requisitionSnapshots[analyticsStore.requisitionSnapshots.length - 1];
 ...
 */
export const recordPlayerDeath = (playerId, playerName, difficulty, mission = 1, reason = 'Failed to extract') => {
  const timestamp = getRelativeTimestamp();
 ...
 */
export const recordLoadoutChange = (playerId, playerName, loadout, slot, itemId, reason = 'draft') => {
  const timestamp = getRelativeTimestamp();
 ...
 */
export const recordGameEvent = (eventId, eventName, eventType, outcome, difficulty, affectedPlayers = []) => {
  const timestamp = getRelativeTimestamp();
 ...
 */
export const recordMissionComplete = (mission, difficulty, starRating, extractedCount = 0, totalPlayers = 1) => {
  const timestamp = getRelativeTimestamp();
...
export const isAnalyticsActive = () => {
  return analyticsStore.runId !== null && analyticsStore.endTime === null;
};
