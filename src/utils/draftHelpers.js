...
export const getWeightedPool = (
    ...
    draftDebugLog('getWeightedPool final result', {
        ...
        superstoreItemsInFinalPool: filteredPool
            .filter((p) => !p.isArmorCombo && p.item?.superstore)
            .map((p) => p.item.id),
        superstoreArmorCombosInFinalPool: filteredPool.filter(
            (p) => p.isArmorCombo && p.armorCombo?.items?.some((a) => a.superstore),
        ).length,
    })
...
export const generateDraftHand = (
    ...
    const handSize =
        customHandSize !== null ? customHandSize : getDraftHandSize(gameConfig.starRating)
        ...
        for (let j = 0; j < adjustedPool.length; j++) {
            ...
            if (randomNum <= 0) {
                ...
                if (poolItem.isArmorCombo) {
                    // eslint-disable-next-line no-console
                    console.log(
                        `[Draft] Selected armor combo: ${poolItem.armorCombo.passive} (${poolItem.armorCombo.armorClass})`,
                        {
    ...
    console.log(
        ...
        hand.map((h) => h.name || `${h.passive} (${h.armorClass})`),
    )
