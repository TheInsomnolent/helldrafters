...
describe('Utils - Draft Helpers', () => {
    ...
    describe('getWeightedPool', () => {
        ...
        it('should boost FIRE items for Bugs faction', () => {
            ...
            const nonFireItem = pool.find(
                ({ item }) =>
                    !item.tags.includes(TAGS.FIRE) && item.rarity === fireItem?.item.rarity,
            )
        ...
        describe('Warbond and Superstore Filtering', () => {
            ...
            it('should only include items from enabled warbonds when warbonds specified', () => {
                ...
                pool.forEach(({ item, armorCombo, isArmorCombo }) => {
                    if (isArmorCombo) {
                        ...
                        const hasAccessibleArmor = armorCombo.items.some(
                            (armor) =>
                                armor.warbond === 'helldivers_mobilize' ||
                                (!armor.warbond && !armor.superstore),
                        )
            ...
            it('should exclude superstore items when includeSuperstore is false', () => {
                ...
                pool.forEach(({ item, armorCombo, isArmorCombo }) => {
                    if (isArmorCombo) {
                        ...
                        const hasSuperStoreOnlyArmor = armorCombo.items.every(
                            (armor) => armor.superstore,
                        )
            ...
            it('should correctly combine warbond filtering and excludedItems', () => {
                ...
                const steeledVetItem = pool.find(
                    ({ item, isArmorCombo }) =>
                        !isArmorCombo &&
                        item.warbond === 'steeled_veterans' &&
