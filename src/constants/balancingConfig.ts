...
export type Subfaction = (typeof SUBFACTION)[keyof typeof SUBFACTION]

export interface SubfactionConfig {
    name: string
    description: string
...
export const SUBFACTION_CONFIG: Record<Subfaction, SubfactionConfig> = {
 ...
 */
export const PLAYER_COUNT_REQ_SCALING: Record<number, number> = {
    1: 1.0, // Solo - baseline
 ...
 */
export const SLOT_LOCK_COST: Record<number, number> = {
    1: 3, // Solo - 3 req
    2: 3, // 2 players - 3 req
    3: 2, // 3 players - 2 req
    4: 2, // 4 players - 2 req
 ...
 * @param subfaction - Enemy subfaction identifier
 * @returns Combined multiplier for requisition awards
 */
export function getRequisitionMultiplier(playerCount: number, subfaction: string): number {
    const playerMultiplier = PLAYER_COUNT_REQ_SCALING[playerCount] || 1.0
    const subfactionMultiplier = SUBFACTION_CONFIG[subfaction as Subfaction]?.reqMultiplier || 1.0
 ...
 * @param playerCount - Number of players (1-4)
 * @param subfaction - Enemy subfaction identifier
 * @returns Combined multiplier for rare and legendary item weights
 */
export function getRareWeightMultiplier(playerCount: number, subfaction: string): number {
    ...
    const subfactionMultiplier =
        SUBFACTION_CONFIG[subfaction as Subfaction]?.rareWeightMultiplier || 1.0
 ...
 * @param faction - Main faction (terminid, automaton, illuminate)
 * @returns Default subfaction identifier
 */
export function getDefaultSubfaction(faction: Faction | string): Subfaction {
    switch (faction) {
        case 'terminid':
 ...
 * @param faction - Main faction (terminid, automaton, illuminate)
 * @returns Array of subfaction identifiers
 */
export function getSubfactionsForFaction(faction: Faction | string): Subfaction[] {
    switch (faction) {
        case 'terminid':
            return [
                SUBFACTION.BUGS_VANILLA,
 ...
 * @param playerCount - Number of players (1-4)
 * @returns Cost in requisition to lock a slot
 */
export function getSlotLockCost(playerCount: number): number {
