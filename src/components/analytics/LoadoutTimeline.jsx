 ...
 */
const getSlotValue = (change, slot) => {
  ...
  return getItemName(change.loadout[slot]);
};
 ...
 */
const buildMergedSegments = (changes, slot, totalMissions, missionTimestamps) => {
...
// Slot row component
const SlotRow = ({ changes, slot, totalMissions, missionTimestamps }) => {
...
const PlayerTimeline = ({ playerId, playerName, changes, totalMissions, playerIndex, missionTimestamps }) => {
  ...
  const slots = ['primary', 'secondary', 'grenade', 'armor', 'booster', 'stratagem0', 'stratagem1', 'stratagem2', 'stratagem3'];
...
const LoadoutTimeline = ({ data, totalMissions = 10, missionStars = [] }) => {
  ...
  const validPlayers = data.filter(player => 
    ...
    player.playerId >= 0 && player.playerId < 4
  );
