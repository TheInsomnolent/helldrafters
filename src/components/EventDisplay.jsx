...
export default function EventDisplay({
    ...
    const getItemById = (itemId) => MASTER_DB.find((i) => i.id === itemId)
    ...
    const isSwapChoice = (choice) => {
        if (!choice || !choice.outcomes) return false
        return choice.outcomes.some(
            (outcome) => outcome.type === OUTCOME_TYPES.SWAP_STRATAGEM_WITH_PLAYER,
        )
    }
    ...
    const isDuplicateChoice = (choice) => {
        if (!choice || !choice.outcomes) return false
        return choice.outcomes.some(
            (outcome) => outcome.type === OUTCOME_TYPES.DUPLICATE_STRATAGEM_TO_ANOTHER_HELLDIVER,
        )
    }
                                ...
                                >
                                    ...
                                    {(!isSwapChoice(selectedChoice) ||
                                        ...
                                        <div style={{ marginBottom: '20px' }}>
                                            ...
                                            >
                                                {isSwapChoice(selectedChoice)
                                                    ? `Step 2: Select HELLDIVER ${eventSourcePlayerSelection + 1}'s Stratagem:`
                                                    : 'Your Stratagems:'}
                                            </div>
                                            <div
                                                style={{
                                    ...
                                    {currentEvent.choices.map((choice, idx) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                onMouseLeave={(e) =>
                                                    canSelect &&
                                                    (e.target.style.backgroundColor = '#F5C642')
                                                }
                                            >
                                                <div
                                                    style={{
